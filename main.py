...
class Library:
    ...
    def add_book(self, title=None, author=None, copies=None):
        if title is None: title = input("Enter book title : ")
        if author is None: author = input("Enter the book author : ")
        if copies is None: copies = int(input("how many copies : "))
    ...
    def add_member(self, name=None, email=None):
        if name is None: name = input("Enter the name :- ")
        if email is None: email = input("please enter the email ")
    ...
    def borrow(self, member_id=None, book_id=None):
        if member_id is None: member_id = input("Enter the mermber ID : ").strip()
        ...
        if book_id is None: book_id = input("enter the book id : ")
    ...
    def return_book(self, member_id=None, book_id=None):
        if member_id is None: member_id = input("Enter the mermber ID : ").strip()
        members = [m for m in Library.data['members'] if m['id'] == member_id]
        if not members:
            print("no such Id exist")
        ...
        if book_id:
            ...
            for i, b in enumerate(member['borrowed']):
                if b['book_id'] == book_id:
                    selected = member['borrowed'].pop(i)
                    break
            else:
